...
var therm_temp = make(map[string]float64)
var therm_hum = make(map[string]float64)
var therm_volt = make(map[string]float64)
var therm_soc = make(map[string]float64)

var v = new(victoriapush.Vic)
...
type influxDataPkg struct {
	metric string
	mac    string
	value  float64
}

func pushVictoriaMetric(dataPkg influxDataPkg) {
	dataPoint := victoriapush.DataPoint{}

	dataPoint.Metric = dataPkg.metric
	dataPoint.Value = dataPkg.value
	dataPoint.Labels = map[string]string{
		"mac": dataPkg.mac,
	}
...
func main() {
	...
	println("vic URL: ", victoriaURL)

	globalLabels := make(map[string]string)
	v = victoriapush.NewVictoriaPush(victoriaURL, globalLabels)
